...
from torch.optim import Adam, AdamW, RAdam, SGD, Optimizer
...
def create_optimizer(param: List, **kwargs) -> Optimizer:
    """ API to create optimizer. """
    name = kwargs['name']
    optimizer_kwargs = kwargs['kwargs']

    if name not in _optimizer:
        raise ValueError(f"Optimizer '{name}' not found.")

    return _optimizer[name](param, **optimizer_kwargs)

def create_scheduler(optimizer: Optimizer, **kwargs):
    """ API to create scheduler. """
    name = kwargs['name']
    ...
    return _scheduler[name](optimizer, **scheduler_kwargs)
